...
agent = model.Agent(epsilon_t)
sim = model.PayLabelSimulator((images_t, labels_t, last_labels_t))
num_actions += 1 # for "pay for label"

observation_last_labels = []
observation_images = []
observation_oracle_labels = []
actions = []
rewards = []
observation_image_t, observation_last_label_t, observation_oracle_label_t = sim.initial_observation()
for t in range(sim.steps-1):
  ...
  observation_oracle_labels.append(observation_oracle_label_t)
  action_t = agent.choose_action((observation_image_t, observation_last_label_t, observation_oracle_label_t))
  #action_t = tf.one_hot(tf.random_uniform([sim.batch_size_t], maxval=num_actions, dtype=tf.int32), num_actions)
  actions.append(action_t)
  reward_t, (observation_image_t, observation_last_label_t, observation_oracle_label_t) = sim.do_step(action_t)
  rewards.append(reward_t)
  agent.learn(reward_t, (observation_image_t, observation_last_label_t, observation_oracle_label_t))

observation_last_labels_t = tf.pack(observation_last_labels, 1)
observation_images_t = tf.pack(observation_images, 1)
observation_oracle_labels_t = tf.pack(observation_oracle_labels, 1)
...
observation_last_labels, observation_images, observation_oracle_labels, actions, rewards = sess.run((observation_last_labels_t, observation_images_t, observation_oracle_labels_t, actions_t, rewards_t), 
                                                                                                    { images_t: images,
                                                                                                      labels_t: labels,
...
for t in range(actions.shape[1]):
  ...
  print 'current image'
  print 'oracle label: ', observation_oracle_labels[b,t]
