...
parser.add_argument('-m', '--model_num', type=int, default=100000, help='the model to eval')
...
print TIME_STEPS
# RUN_NAME = 'pay_%s_%s_%dcpe_%dlbls_%dc_%03deps_%02dkcrclm_%02dbatch_qrl'%(args.dataset, args.cell_type, CLASSES_PER_EPISODE, NUM_LABELS, data.NUM_CLASSES, int(EPSILON*100), CURRICULUM_STEP_SIZE/1000, BATCH_SIZE)
# LOG_DIR = 'logs_pay/'
...
sess = tf.Session()
#sess.run(init_op)
print 'loading model ' + args.model_dir+'/model_%d.ckpt'%(args.model_num)
saver.restore(sess, args.model_dir+'/model_%d.ckpt'%(args.model_num))

#train_writer = tf.train.SummaryWriter(LOG_DIR+"%s/train"%(RUN_NAME), sess.graph)
test_writer = tf.train.SummaryWriter(args.model_dir+"/eval_%d"%(args.model_num), sess.graph)

#for e in range(NUM_EPISODES):
for e in range(args.model_num, args.model_num+100000):
  ...
  if not (e+1)%EPISODES_PER_LOG:
    ...
    true_labels = np.argmax(labels[:,:-1], axis=2)
    predictions, rewards, loss = sess.run([predictions_t, rewards_t, loss_t], {images_t: images, labels_t: labels, last_labels_t: last_labels, epsilon_t: 0.0})
    ...
    print e+1, loss, reward_max, accuracy_max, accuracy_1st, accuracy_2nd, accuracy_5th, accuracy_10th, 'cpe=%d num_labels=%d'%(CLASSES_PER_EPISODE, NUM_LABELS)
